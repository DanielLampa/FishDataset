...
def count(founded_classes,im0):
  ...
  for i, (k, v) in enumerate(founded_classes.items()):
    ...
    cv2.putText(im0, str(a) ,(int(align_right),align_bottom), cv2.FONT_HERSHEY_SIMPLEX, 0.50,(0,0,255),1,cv2.LINE_AA)
...
def total(founded_classes,im0,total_last):
  ...
  totCountedObjectFrame=sum(total_last)
  totCountedObjectCFrame=total_last[-1]
...
if __name__ == '__main__':
    st.title('fishDA WEB APP')
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default='weight\COVERTED_MODEL_CLASSIFIER.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='Image\Tilapia.jpg', help='source')  # file/folder, 0 for webcam
    ...
    if source_index == 0:
        ...
        if uploaded_file is not None:
            ...
            with st.spinner(text='Loading...'):
                ...
                picture = picture.save('data\images'+name) 
                opt.source = (f'data\images'+name)
        else:
            is_valid = False
    else:
        ...
        if uploaded_file is not None:
            ...
            with st.spinner(text='Loading...'):
                ...
                name=uploaded_file.name
                with open(os.path.join("data", "videos", name), "wb") as f:
                    f.write(uploaded_file.getbuffer())
                opt.source = (f'data\videos'+name)
        else:
            is_valid = False
    ...
    #checkbox
    confidence=st.sidebar.slider('Confidence Threshold', 0.0, 0.99, 0.25)#minumum confidence treshold
